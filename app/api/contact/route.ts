...
function validatePayload(payload: ContactPayload): string | null {
  ...
  if (!payload.selfEmployed && !payload.company) {
...
async function checkRateLimit(ip: string) {
  const sinceDate = new Date(Date.now() - RATE_LIMIT_WINDOW_HOURS * 60 * 60 * 1000).toISOString();
  ...
  try {
    ...
    if (!allowed) {
      return NextResponse.json(
        { message: "Daily contact limit reached. Please try again after 24 hours." },
        { status: 429 }
      );
    }
    ...
    const { error: contactInsertError } = await supabase
      ...
      .insert({
        ...
        phone: payload.phone,
        service_topic: payload.serviceTopic,
    ...
    return NextResponse.json(
      { success: false, message: "Unable to process your request at the moment." },
      { status: 500 }
    );
  }
}
